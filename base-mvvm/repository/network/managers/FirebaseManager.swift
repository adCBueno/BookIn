...
class FirebaseManager {
    ...
    func getBookDetails(completion: @escaping ( Result<[BookListDetailModel]?, Error>) -> Void  ) {
        
        db.collection("media").getDocuments { querySnapshot, error in
            guard error == nil else { return completion(.failure(error!)) }
            guard let documents = querySnapshot?.documents, !documents.isEmpty
                   else { return completion(.failure(FirebaseErrors.InvalidUser)) }
            
            var bookList = [BookListResponse]()
            for document in documents {
                ...
                if let item = try? FirestoreDecoder().decode(BookListResponse.self, from: document.data()) {
                    bookList.append(item)
                }
            }
            let bookListDetailModel = self.convertFirebaseModelToBookListDetailModel(bookList: bookList)
            completion(.success(bookListDetailModel))
        }
    }
    
    func convertFirebaseModelToBookListDetailModel(bookList: [BookListResponse]) -> [BookListDetailModel] {
        var bookListModel: [BookListDetailModel] = []
        for bookList in bookList {
            let bookDetails = BookListDetailModel(author: bookList.author,
                                                  imageName: bookList.image,
                                                  bookTitle: bookList.title ?? "NA",
                                                  bookDescription: bookList.sinopsis,
                                                  bookUpvoteCount: "",
                                                  category: bookList.category,
                                                  createdAt: bookList.createdAt,
                                                  updatedAt: bookList.updatedAt,
                                                  comment: bookList.comment,
                                                  idOwner: bookList.idOwner)
            bookListModel.append(bookDetails)
        }
